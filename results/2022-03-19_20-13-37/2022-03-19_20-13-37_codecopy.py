...
class Strat1(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.cnt = 0
        self.ntrades = 0
        self.daydelta = datetime.timedelta(days=1)
        self.lastday = dict()
        self.o = dict()
        self.dateopen = dict()
        ...
        for i, d in enumerate(self.datas):
            if(d._name.split('_')[0]=='RSP'):
                continue

            self.inds[d] = dict()
            
            self.lastday[d] = datetime.datetime.strptime(
                d._name.split('_')[1],'%Y-%m-%d').date()
            ...
            self.dateopen[d] = self.lastday[d] # Initializes a variable to 
                                               # hold the date that the last 
                                               # trade was opened
        ...
        for i, d in enumerate(self.datas):
            if len(self.datas[i])>2: # Check that there is data loaded
                ...
                if (pos and not self.o.get(d, None)):
                    # Close out if ticker is about to end
                    if dt>=self.lastday[d]-self.daydelta:
                        self.o[d] = [self.close(data=d,valid=None)]
                        # self.holding[d] = 0
                    # Close out if indicator signals
                    elif (self.inds[d]['merit'].l.exit[0] >= 90 and
                            dt>=(self.dateopen[d]+self.daydelta)):
                        self.o[d] = [self.close(data=d,valid=None)]
                        # self.holding[d] = 0
                        
                # if NOT IN a position and no pending orders
                elif (not pos and not self.o.get(d, None) 
                        and dt<(self.lastday[d]-self.daydelta)
                        and self.ntrades < 10
                        and self.inds[d]['merit'].l.mb[0] >= 85):
...
if __name__ == '__main__':
    ...
    mnt_folder = ('C:/Users/rtm/Documents/Personal/Stock_Market/Python/' +
                    'universe_data/sp500/stock_data/IQFeed/' + 
                    'daily/')
    ...
    for filename in os.listdir(mnt_folder):
        if filename.endswith(".csv"):
            ...
            if ((date1<=sdate and date1<=edate and 
    ...
    # Add a sizer
    cerebro.addsizer(FixedValueSizer, value=1500)
    ...
    f = (r'C:/Users/rtm/Documents/Personal/Stock_Market/Python/' + 
          'universe_data/sp500/stock_data/benchmark/RSP/' + 
          '1_0_0_0_RSP_2013-03-01_2020-04-23.csv')
    rsp_bench = pd.read_csv(f,index_col='Date',parse_dates=True)
    rsp_bench.index = rsp_bench.index.date
    rsp_bench['Ret'] = rsp_bench['Close'].loc[
        (rsp_bench.index >= sdate.date()
              + datetime.timedelta(days=Strat1.params.smalong*1.5)) &
        (rsp_bench.index <= edate.date())]
    rsp_bench['Ret'] = (rsp_bench['Ret'].pct_change().add(1+0.002/252)
    ...
    for x in fig[0][0].axes[1].lines[0].get_xdata():
        ...
        dt = (datetime.datetime.strptime(
                fig[0][0].axes[1].format_xdata(x),'%Y-%m-%d').date())
        ypt.append(rsp_bench['Ret'][rsp_bench.index==dt].values[0])
