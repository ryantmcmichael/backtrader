...
* Figure out why buy/sell vertical bars are incorrect
* Include premarket minute bars (this is just a yahoo issue)
* Figure out how to do Stop-Loss
'''
...
class Strat1(bt.Strategy):
    ...
    def __init__(self):
        ...
        for i, d in enumerate(self.datas):
            ...
            self.inds[d]['uo'] = bt.indicators.UltimateOscillator(d,
                                           ...
                                           p2 = self.params.uop2,
                                           p3 = self.params.uop3,
                                           upperband = self.params.uoub,
                                           lowerband = self.params.uolb)
            ...
            if len(self.datas[i])>2: # Check that there is data loaded
                ...
                if (pos and not self.o.get(d, None)):
                    ...
                    # Note: Could force to hold for at least 1 day
                        ...
                        (self.inds[d]['smashort'] < self.inds[d]['smalong'])
                        or (self.data.close < self.data.open[-2]*0.97)
                        or (self.inds[d]['merit'] < 2)):
                        self.o[d] = [self.close(data=d,valid=None)]
                        # self.holding[d] = 0
    ...
    def notify_order(self, order):
        ...
        # else:
        #     self.sell(exectype=bt.Order.StopTrail, trailamount=self.params.trail)
...
if __name__ == '__main__':
    ...
    ys=2022;ms=3;ds=13
    ye=2022;me=3;de=19
    mnt_folder = ('C:/Users/rtm/Documents/Stock_Market/Python/' +
    ...
    bench = bench.loc[
        (bench.index >= sdate) &
        (bench.index <= edate)]
    bench['Ret'] = (bench['Close'].pct_change().add(1).
    ...
    for x in fig[0][0].axes[1].lines[0].get_xdata():
        ...
        dt = (datetime.datetime.strptime(
                fig[0][0].axes[1].format_xdata(x),'%Y-%m-%d %H:%M'))
