 ...
 * Need to narrow down data to only show days I traded certain tickers
        ...
        for i, d in enumerate(self.datas):
            if len(self.datas[i])>2: # Check that there is data loaded
                ...
                if (pos and not self.o.get(d, None)):
                    # Close out if ticker is about to end (OPTION)
                    ...
                    if (self.inds[d]['smashort'] < self.inds[d]['smalong']
                          # and dt>=(self.dateopen[d]+self.daydelta)
                          ):
                        self.o[d] = [self.close(data=d,valid=None)]
                        # self.holding[d] = 0
                ...
                # if NOT IN a position and no pending orders
                        ...
                        # and dt<(self.lastday[d]-self.daydelta) OPTIONAL
                        and self.ntrades < 10
